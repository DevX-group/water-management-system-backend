...
public interface ConversationRepository extends JpaRepository<Conversation, UUID> {
        ...
        @Query(value = "select c from InternalChatConversation c join c.participants cp where cp.user = :user " +
                        ...
                        +
                        "order by c.updatedAt desc")
        List<Conversation> findByUserAndOtherParticipantNameLike(@Param("user") User user,
        ...
        @Query(value = "select c from InternalChatConversation c join c.participants cp where cp.user = :user " +
                        ...
                        +
                        "order by c.updatedAt desc")
        List<Conversation> findByUserAndOtherParticipantIdLike(@Param("user") User user,
                        @Param("search") String search);
}
